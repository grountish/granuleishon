...
import {
  ...
  editGen4NoteCellFromElement,
  ensureGen4LockStepActive,
  gen4ApplyStepBtn,
  gen4CycleCondition,
  gen4CycleProbability,
  gen4StepConditionMet,
  generateGen4Arp,
  generateGen4Bass,
  generateGen4Euclid,
  generateGen4Melody,
  generateGen4Polyrhythm,
  getGen3StepLockTarget,
  getGen4BaseMidi,
  getGen4LaneMidiRange,
  ...
  selectGen4LockStep,
  toggleGen4LockStep,
} from './instruments/gen4/note-editor.js';
import {
  gen4Ui,
  gen4ControlBindings,
  ...
  gen4NoteDrawState,
  gen4FillState,
} from './instruments/gen4/editor-state.js';
import { GEN3, GEN3_PARAM_DEFS, gen3ControlBindings } from './instruments/gen3/state.js';
...
import {
  ...
  eqYToDb,
  drawMasteringEq,
  setMasteringEqReadout,
} from './master/eq-plot.js';
import {
  MASTERING,
  MASTERING_MODULE_IDS,
  MASTERING_DEFAULT_PARAMS,
  MASTERING_FACTORY_PRESETS,
    ...
    const control = makeControlRow(
      ...
      (v) => {
        ...
        if (stepLocks) {
          ...
          gen4ApplyStepBtn(gen4Ui.lockSelection.ci, gen4Ui.lockSelection.si);
          refreshGen4LockEditor();
          return;
        }
        GEN3[p.key] = v;
...
function applyGen4Preset(ci, presetIndex) {
  ...
  if (ci === gen4Ui.selectedNoteChannel && gen4Ui.editorMode === 'notes') {
...
function applyGen4KitPreset(presetIndex) {
  ...
  if (gen4Ui.editorMode === 'notes') refreshGen4NoteEditor();
  refreshGen4LockEditor();
  emit('state');
}
...
function gen4RefreshStepDisplay() {
  ...
  if (gen4Ui.editorMode === 'notes') refreshGen4NotePlayhead();
}
...
function gen4SetStepCount(n, { duplicateOnExpand = true } = {}) {
  ...
  if (gen4Ui.editorMode === 'notes') {
    for (let si = 0; si < 32; si++) refreshGen4NoteStep(si);
    gen4Ui.noteRollEl?.querySelectorAll('.drum-note-step-number').forEach((el, si) => {
...
function fitGen4NotesToScale() {
  ...
  for (let si = 0; si < 32; si++) gen4ApplyStepBtn(gen4Ui.selectedNoteChannel, si);
  renderGen4NoteRoll();
  const label = GEN4_DEFS[gen4Ui.selectedNoteChannel]?.label || 'lane';
  setStatus(
    changed > 0
...
function setGen4EditorMode(mode) {
  ...
  gen4Ui.editorMode = mode;
  if (mode === 'notes' && gen4Ui.noteCellEls.every((cells) => cells.size === 0)) {
    renderGen4NoteRoll();
  }
  refreshGen4NoteEditor();
  refreshGen4LockEditor();
...
function buildGen4NoteEditor() {
  ...
  pencil.addEventListener('click', () => {
    gen4Ui.notePencilEnabled = !gen4Ui.notePencilEnabled;
    pencil.classList.toggle('active', gen4Ui.notePencilEnabled);
    pencil.setAttribute('aria-pressed', String(gen4Ui.notePencilEnabled));
  });
  ...
  roll.addEventListener('pointerdown', (event) => {
    if (!gen4Ui.notePencilEnabled || event.button !== 0) return;
  ...
  gen4Ui.noteEditorEl = editor;
  gen4Ui.noteRollEl = roll;
  return editor;
}
...
function buildDrumPanel() {
  ...
  [
    ...
    btn.className = 'drum-editor-mode-btn';
    btn.textContent = label;
    btn.classList.toggle('active', gen4Ui.editorMode === mode);
  ...
  gen4Ui.lockClearBtn = document.createElement('button');
  gen4Ui.lockClearBtn.type = 'button';
  gen4Ui.lockClearBtn.className = 'drum-lock-clear-btn';
  gen4Ui.lockClearBtn.textContent = 'CLR';
  gen4Ui.lockClearBtn.title = 'Clear parameter locks on the selected step';
  gen4Ui.lockClearBtn.disabled = true;
  gen4Ui.lockClearBtn.hidden = true;
  gen4Ui.lockClearBtn.addEventListener('click', clearSelectedGen4Locks);
  actions.appendChild(gen4Ui.lockClearBtn);
...
// so an old project never inherits the previous project's chain. The state
// itself (params, pristine defaults, factory chains) lives in master/state.js.
...
function buildBusFx(busId) {
  ...
  // Each reorderable unit builds its own graph inside the rack's wet/dry
  // scaffold — see fx/registry.js and the unit modules.
  const units = {};
  DEFAULT_FX_ORDER.forEach((id) => {
    units[id] = buildUnitNodes(FX_UNITS_BY_ID.get(id), ac, st[id], (key) =>
      getBaseFxValue(id, key, busId),
    );
  });
